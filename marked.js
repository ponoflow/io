...
class MarkedClass {
    constructor() {
        this.rules = [
            ...
            { pattern: /\*\*(\d+\.)\*\*/g, replacement: '<strong>$1</strong>' },  // For numbered lists
            ...
            // Custom CSS
            { pattern: /\{style:([^}]+)\}([\s\S]*?)\{\/style\}/g, replacement: (_, style, content) => `<div style="${style}">${content}</div>` },
            
            // Paragraphs
            { pattern: /^(?!<[a-z])(.+)$/gm, replacement: '<p>$1</p>' }
        ];
    }
...
var ezMarkup = (el, text)=>{
	el.innerHTML=marked.parse(text||el.innerHTML);
	Prism.highlightAll();
};

document.head.innerHTML+=`<style>code[class*=language-],pre[class*=language-] {
    color: #ccc;
    background: 0 0;
...
.token.inserted {
    ...
    function decodeHexEntities(text) {
        return text.replace(/\\x([0-9A-Fa-f]{2})/g, (match, hex) => {
            return String.fromCharCode(parseInt(hex, 16));
        }).replace(/<!\-\-/g, '<').replace(/\-\->/g, '>');
    }
    function matchCount(mainStr, obj) {
        let maxCount = 0;
        let result = null;
        let maxcols = [];
        var maxval=''
        ...
        for (const key in obj) {
            result=mainStr.split(key).length-1;
            cnts[key]=result;
            if(result>maxCount){
                maxcols=[key];
                maxval=obj[key];
        ...
        return {maxval ,cnts, maxcols, obj};
    }
    function escapeMarkdown(markdown) {
        return markdown.replace(/&/g, '&amp;').replace(/</g, '&lt;')
            .replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;');
    }
    function indentCodeMD(code) {
        var rtn="",i=-1;
        for (let line of code.split("```")){i++;
            if(i%2)line="```"+indentCode(line)+"```";
            rtn+=line;
        }
        return rtn;
    }
    function indentCode(code) {
        ...
        for (let line of code.split("\n")) {
            var info=matchCount(line,{"{":1,"}":-1});
            var cha=info.maxcols.pop()||'';
            match=info.maxval||0;
            if((cha=='}' || match==-1)){
                indent+=match;
                newCode += "    ".repeat(min(indent+open))+line+"\n";
            }else if((cha=='{' || match==1)){
                newCode+="    ".repeat(min(indent+open))+line+"\n";
                indent+=match;
            }
            open=info.maxcols[0]=='}'&& cha?1:0;
        }
        return newCode;
    }
function doMarkUp(el=".markup", string=''){  
    var target=$(el);
    var item,i=-1;
    while(item=target[++i]){
        let buffer = '';
