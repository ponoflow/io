...
style.textContent=`.hljs{background:#2b2b2b;color:#f8f8f2}.hljs-comment,.hljs-quote{color:#d4d0ab}.hljs-deletion,.hljs-name,.hljs-regexp,.hljs-selector-class,.hljs-selector-id,.hljs-tag,.hljs-template-variable,.hljs-variable{color:#ffa07a}.hljs-built_in,.hljs-link,.hljs-literal,.hljs-meta,.hljs-number,.hljs-params,.hljs-type{color:#f5ab35}.hljs-attribute{color:gold}.hljs-addition,.hljs-bullet,.hljs-string,.hljs-symbol{color:#abe338}.hljs-section,.hljs-title{color:#00e0e0}.hljs-keyword,.hljs-selector-tag{color:#dcc6e0}.hljs-emphasis{font-style:italic}.hljs-strong{font-weight:700}@media screen and (-ms-high-contrast:active){.hljs-addition,.hljs-attribute,.hljs-built_in,.hljs-bullet,.hljs-comment,.hljs-link,.hljs-literal,.hljs-meta,.hljs-number,.hljs-params,.hljs-quote,.hljs-string,.hljs-symbol,.hljs-type{color:highlight}.hljs-keyword,.hljs-selector-tag{font-weight:700}}`;
document.head.appendChild(style);
async function* mdjs(inputStream) {
  // Assuming inputStream is a ReadableStream (adjust if needed)
  ...
  try {
    while (true) {
      ...
      buffer += new TextDecoder("utf-8").decode(value); // Assuming UTF-8 encoding

      // Process complete Markdown blocks
      const blocks = buffer.split("\n\n"); 
      buffer = blocks.pop(); // Keep the last (potentially incomplete) block

      for (const block of blocks) {
        yield markdown.render(block); 
      }
    }

    // Process any remaining content in the buffer
    if (buffer) {
      yield markdown.render(buffer);
    }
...
// Example usage (adjust based on your input and output streams):
const input = /* ... your ReadableStream */;
const output = /* ... your WritableStream */; 

(async () => {
  for await (const chunk of mdjs(input)) {
